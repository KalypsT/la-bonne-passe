...
// Joueur actif, classique à 3 rendez-vous, 35 nuits, 10 graines. Sans carte : le voisin, que le burlesque réveille, et les imprévus sont gardés à part.
...
function jouer(nom: string, theme: string | null, tendances: string[] = []) {
  parties.set(nom, GRAINES.map((graine) => simuler({ graine, offre: 'classique', rdvMax: 3, nuits: 35, tendances, theme: vendrediSamedi(theme), cartes: false }).nuits));
}
